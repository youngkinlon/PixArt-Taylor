...
from diffusion.model.nets.cache_fuctions import cal_type
from diffusion.model.nets.taylor_utils import derivative_approximation, taylor_formula
...
from diffusion.model.nets.taylor_utils import derivative_approximation, taylor_formula, taylor_cache_init
...
class PixArtBlock(nn.Module):
    ...
    def forward(self, x, y, t, mask=None, cache_dic=None,current=None,**kwargs):
        B, N, C = x.shape
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (self.scale_shift_table[None] + t.reshape(B, 6, -1)).chunk(6, dim=1)
        if current['type']=='full':
            current['module']='self-attn'
            taylor_cache_init(cache_dic,current)
            self_attn=self.attn(t2i_modulate(self.norm1(x), shift_msa, scale_msa)).reshape(B, N, C)
            derivative_approximation(cache_dic,current,self_attn)
            x = x + self.drop_path(gate_msa * self_attn)
            current['module']='cross-attn'
            taylor_cache_init(cache_dic,current)
            cross_attn=self.cross_attn(x,y,mask)
            derivative_approximation(cache_dic,current,cross_attn)
            x = x + cross_attn
            current['module']='mlp'
            taylor_cache_init(cache_dic,current)
            mlp = self.mlp(t2i_modulate(self.norm2(x), shift_mlp, scale_mlp))
            derivative_approximation(cache_dic,current,mlp)
            x = x + self.drop_path(gate_mlp * mlp)
        elif current['type']=='taylor':
            current['module'] = 'self-attn'
            x=x+self.drop_path(gate_msa * taylor_formula(cache_dic,current))
            current['module']='cross-attn'
            x = x + taylor_formula(cache_dic,current)
            current['module']='mlp'
            x = x + self.drop_path(gate_mlp * taylor_formula(cache_dic,current))
        return x
...
class PixArt(nn.Module):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None,cache_dic=None,current=None, **kwargs):
        """
        ...
        for layeridx, block in enumerate(self.blocks):
            ...
            x = auto_grad_checkpoint(block, x, y, t0, y_lens,cache_dic,current)  # (N, T, D) #support grad checkpoint
        x = self.final_layer(x, t)  # (N, T, patch_size ** 2 * out_channels)
        x = self.unpatchify(x)  # (N, out_channels, H, W)
        return x

    def forward_with_dpmsolver(self, x, timestep, y, mask=None,cache_dic=None,current=None, **kwargs):
        """
        dpm solver donnot need variance prediction
        """
        # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb
        print("dpmsolver",current)
        print("dpmsolver all kwargs:", kwargs.keys())

        model_out = self.forward(x, timestep, y, mask,cache_dic=cache_dic,current=current)
        return model_out.chunk(2, dim=1)[0]

    def forward_with_cfg(self, x, timestep, y, cfg_scale, mask=None,cache_dic=None,current=None, **kwargs):
        """
        Forward pass of PixArt, but also batches the unconditional forward pass for classifier-free guidance.
        """
        # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb
        print("forward_with_cfg",current)
        half = x[: len(x) // 2]
